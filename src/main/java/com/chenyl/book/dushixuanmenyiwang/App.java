package com.chenyl.book.dushixuanmenyiwang;
...
{//http://www.du1du.net
    ...
    public static void main( String[] args ) throws IOException {
        ...
        Document doc= Jsoup.connect(BASE_URL+"/43_43777/").ignoreContentType(true).get();

        AtomicBoolean flag = new AtomicBoolean(false);
        doc.select(".listmain a").stream().forEachOrdered((element)->{
            Document doc1= null;
            try {
                if(element.text().contains("第七百一十二章"))
                    flag.set(true);
                if(flag.get())
    ...
    private static void analysChapter(Element element,String path){
       try{
           ...
           TxtUtil.writeTxtFile("\n\n"+element.text()+"\n\n"+element1.text().replaceAll("&1t;/p>",""), new File(path));
